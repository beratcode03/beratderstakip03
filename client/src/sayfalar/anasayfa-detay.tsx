...
const CenteredWelcomeSection = memo(function CenteredWelcomeSection() {
  ...
  // Sakarya Serdivan  için tarih ve saat formatlama - memoize edildi
...
export default function Homepage() {
  ...
  const sendReportMutation = useMutation({
    mutationFn: () => {
      ...
      const completedTopicsCount = completedGeneral.length + completedBranch.length + completedQuestion.length;
      
      return apiRequest("POST", "/api/reports/send", { 
        isManualRequest: true,
        completedTopicsCount,
        completedTopicsData: [...completedGeneral, ...completedBranch, ...completedQuestion]
      });
    },
    onSuccess: () => {
  ...
  const getActivitiesForDate = useCallback((date: Date) => {
    ...
    const totalQuestionsCount = dayQuestionLogs.reduce((sum, log) => {
